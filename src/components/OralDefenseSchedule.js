...
import {Alert, Col, Divider, Row, Select, Table, DatePicker, TimePicker, Button, message} from 'antd';
...
const OralDefenseSchedule = (project) => {
    ...
    let oralDefenseDay = null;
    let oralDefenseTime = null;
    ...
    const saveDate = () => {
        ...
        console.log("Datos antes de armar", project.project);
        try {
            const schedule = {
    ...
    return (
        <div>
            ...
            <Row type='flex' justify='center' style={{paddingTop: 20, paddingBottom: 50}}>
                <Col span={10} style={{paddingLeft: 330}}>
                    <DatePicker placeholder={'Seleccionar día'} onChange={onChangeDay}/>
                </Col>
                <Col span={10}>
                    <TimePicker placeholder={'Seleccionar hora'} onChange={onChangeHour}/>
                </Col>
            </Row>

            <Row type='flex' justify='center'>
                <Col span={10} style={{paddingLeft: 280}}>
                    <Button onClick={saveDate} disabled={daySelected}>
                        <FieldTimeOutlined/>
                        Enviar fecha tentativa
                    </Button>
                </Col>
                <Col span={10} style={{paddingLeft: 20}}>
                    <Button style={{backgroundColor: "#034c70", color: "white"}} disabled={daySelected}>
                        <CheckOutlined/>
